...
#
# This differs from R.cgi in that it can be used for POST submissions
# where the size of the data being posted is too large for the shell
# and would end up with an error of the form 
#   cat  argument list too long
# This calls Rscript <foo.R> <&0 to redirect the 
#
#
...
POST)
# cat > /tmp/R_data; FORM_DATA="/tmp/R_data" 
#  exec 3<&0
;;
esac
export FORM_DATA
...
#$PATH_TO_R --no-restore --no-save --no-readline --slave <&3
#$PATH_TO_R --no-restore --no-save --no-readline --slave <&3

R_SCRIPT=`basename $PATH_INFO`

THE_RESULTS=`Rscript $PATH_TRANSLATED <&0`

#THE_RESULTS=`case $R_NICE in
#NONE) $Rcall < $PATH_TRANSLATED ;;
#NONE) echo $PATH_TRANSLATED | $Rcall ;;
#none) $Rcall < $PATH_TRANSLATED ;;
